"""Backend de escalado de vídeo con Real-ESRGAN Compact.

Es el nivel rápido: medido en este hardware da ~90 fps a salida 1080p con
51 MiB de VRAM, así que convive con los modelos de lenguaje sin desalojarlos.
Para el nivel de máxima calidad (difusión) ver ``flashvsr_backend``.
"""
...
# Medido sobre una RTX 3090 con salida 1080p: 51 MiB con lote de 1, 394 MiB con
# lote de 8 y 1569 MiB con lote de 32. Se reserva con holgura para el lote por
# defecto y los buffers de ffmpeg, redondeando hacia arriba.
_VRAM_ESTIMATE_MB = 1024


class UpscalerBackend(BackendInterface):
    @classmethod
    ...
    def install_spec(self) -> BackendInstallSpec:
        return BackendInstallSpec(
            oci_image="ghcr.io/ocabra/backend-upscaler",
            ...
            # El worker trocea y recodifica con ffmpeg, así que lo necesita en
            # el contenedor aunque el modelo sea puro torch.
            apt_packages=["ffmpeg"],
            estimated_size_mb=3000,
            display_name="Upscaler (Real-ESRGAN Compact)",
            description=(
                "Escalado y restauración de vídeo 4x con Real-ESRGAN Compact. "
                "Rápido y de VRAM mínima: apto para vídeos completos."
            ),
            tags=["Video", "GPU", "CUDA"],
        )
    ...
    def _worker_script_path(self) -> Path:
        ...
        script = repo_root / "workers" / "upscaler_worker.py"
    ...
    async def load(self, model_id: str, gpu_indices: list[int], **kwargs) -> WorkerInfo:
        ...
        process = await asyncio.create_subprocess_exec(
            *cmd,
            ...
            stderr=open(f"/tmp/upscaler_worker_{model_id.replace('/', '_')}.log", "wb"),
            env=env,
        )

        try:
            await self._wait_until_healthy(port=port, process=process, timeout_s=120)
        except Exception:
    ...
    async def unload(self, model_id: str) -> None:
        ...
        if process is None:
            return
        await self._terminate_process(process)
        logger.info("upscaler_worker_unloaded", model_id=model_id)
    ...
    async def forward_stream(
        ...
        raise RuntimeError("Upscaler backend does not support streaming")
    ...
    async def _wait_until_healthy(
        ...
        while time.monotonic() < deadline:
            if process.returncode is not None:
                raise RuntimeError(f"Upscaler worker exited with code {process.returncode}")
            if await self._health_check_port(port):
                return
            await asyncio.sleep(1)
        raise TimeoutError(f"Upscaler worker health check timed out after {timeout_s}s")
