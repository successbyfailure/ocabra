...
- **diffusers e2e** ✅ (2026-04-25): worker extendido con `from_single_file()`; carga el `sd_xl_base_1.0.safetensors` ya presente y genera 1024×1024 PNG en ~117 s. Spec ahora pinea `transformers<5.0` para evitar el crash `CLIPTextModel.text_model`. **Bug abierto**: en endpoints OpenAI el federation hook corre antes de `resolve_profile`, por lo que `model="<profile_id>"` da 404; usar canonical model_id mientras tanto.
